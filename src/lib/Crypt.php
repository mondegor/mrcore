...
{
    /**
     * Имена наборов символов участвующих в генерации пароля.
     */
     ...
     *
     * @var  array [int => [int, bool, int, string], ...]
     *             [PW_* => [successively max, first or last, letters len, letters], ...]
     */
    private const PW_CHAR_SET = array
    (
        // vowels
    ...
    /**
     * Возвращается хеш указанной строки. Длина хеша равна 64 символа.
     *
     * @param      string  $string
     * @param      string  $salt
     * @return     string
    ...
    {
        return hash('sha256', uniqid(getmypid() . '|' . random_int(1, PHP_INT_MAX), true) . '|' . microtime() . '|' . $salt);
    }
    ...
    public static function generatePassword(int $length, int $charType = self::PW_ALL): string
    {
        ...
        foreach (self::PW_CHAR_SET as $type => $params)
        {
            if ($charType & $type)
            {
   ...
   /**
    * Возвращается временное ключевое слово используемое для шифрования данных.
    *
    * @param      string  $filePath
    * @param      int  $expiry (в секундах, 900 сек. = 15 мин. = 15 * 60)
    * @param      bool  $regenerate
    * @return     string
    */
    public static function getKeyword(string $filePath, int $expiry = 900, bool $regenerate = false): string
    {
        if (!$regenerate && is_file($filePath))
        {
            include($filePath);
        }
        ...
        if (!isset($keyword, $time) || $time < time() - $expiry)
        {
            ...
            if ($fh = fopen($filePath, 'ab'))
            {
                flock($fh, LOCK_EX);
                ftruncate($fh, 0);
                fwrite($fh, sprintf('<?' . 'php $keyword = \'%s\'; $time = %u', $keyword, time()));
                fflush($fh);
                flock($fh, LOCK_UN);
                fclose($fh);
            }
     ...
     * @return     string
     */
    protected static function _encryptionXOR(string $str, string $keyword, string $salt): string
    {
        $len = strlen($str);
        $gamma = '';
        $n = $len > 100 ? 8 : 2;
